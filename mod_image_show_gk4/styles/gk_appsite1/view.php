	...
	<div class="gkIsTextData">
		<?php for($i = 0; $i < count($this->config['image_show_data']); $i++) : ?>
			...
			<?php 
                ...
                if($this->config['image_show_data'][$i]->type == "k2"){
                	$link =  $this->articlesK2[$this->config['image_show_data'][$i]->artK2_id]["link"];
					$content = str_replace('[ampersand]', '&',str_replace('[leftbracket]', '<', str_replace('[rightbracket]', '>', $this->articlesK2[$this->config['image_show_data'][$i]->artK2_id]["title"])))."<small>".$this->articlesK2[$this->config['image_show_data'][$i]->artK2_id]["content"]."</small>";
                 } else {
				  // creating slide title
					$link = ($this->config['image_show_data'][$i]->type == "text") ? $this->config['image_show_data'][$i]->url : $this->articles[$this->config['image_show_data'][$i]->art_id]["link"];	
					$text = ($this->config['image_show_data'][$i]->type == "text") ? $this->config['image_show_data'][$i]->name : $this->articles[$this->config['image_show_data'][$i]->art_id]["title"];
					$text2 = ($this->config['image_show_data'][$i]->type == "text") ? $this->config['image_show_data'][$i]->content : $this->articles[$this->config['image_show_data'][$i]->art_id]["content"];
					
					$content = str_replace('[ampersand]', '&',str_replace('[leftbracket]', '<', str_replace('[rightbracket]', '>', $text)))."<small>".str_replace('[ampersand]', '&',str_replace('[leftbracket]', '<', str_replace('[rightbracket]', '>', $text2)))."</small>";
					;
					}
