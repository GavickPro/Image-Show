...
class GKIS_University_Image {
	...
	*/
	function translateName($name, $mod_id) {
		$name = GKIS_University_Image::getRealPath($name);
	...
	*/
	...
	function createThumbnail($path, $config, $width, $height, $image_bg, $image_stretch, $quality) {
		if(GKIS_University_Image::checkCache(GKIS_University_Image::translateName($path,$config['module_id']), $config['last_modification'], $config['module_id'])){
			return TRUE;	
		}else{
			// importing classes
			...
			$cache_dir = JPATH_ROOT.DS.'modules'.DS.'mod_image_show_gk4'.DS.'cache'.DS;
			// file path
			$file = GKIS_University_Image::getRealPath($path);
			// filename
			$filename = GKIS_University_Image::translateName($path,$config['module_id']);
