...
<div id="gkIs-<?php echo $this->config['module_id'];?>" class="gkIsWrapper-gk_photo">
	...
	<?php if($this->config['image_show_data'][$i]->published) : ?>
		...
		<figure data-url="<?php echo $path; ?>" data-link="<?php echo $link; ?>" data-zindex="<?php echo $i+1; ?>" data-title="<?php echo $title; ?>">
			<figcaption<?php if($this->config['config']->gk_photo->gk_photo_dark_theme == '1') : ?> data-theme="dark"<?php endif; ?>>
			<?php if($this->config['config']->gk_photo->gk_photo_show_title_block && $title != '') : ?>	
	...
	<?php if($this->config['config']->gk_photo->gk_photo_show_pagination): ?>	
