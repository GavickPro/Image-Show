window.addEvent("load",function(){
	setTimeout(function() {
	    $$(".gkIsWrapper-gk_bluap").each(function(el){
	        ...
	        // text block position
	        $G['text_pos'] = wrapper.getAttribute('data-textpos');
	        // blank flag
	        $G['blank'] = false;
	        // load the images
	        wrapper.getElements('.figure').each(function(elm, i){
	            var newImg = new Element('img',{
	                "title": elm.getAttribute('data-title'),
	                "alt": elm.getAttribute('data-alt'),
	                "class": 'gkIsSlide',
	                "style": 'z-index: ' + elm.getAttribute('data-zindex') + ';',
	                "src": elm.getAttribute('data-url')
	            });
	            links[i] = elm.getAttribute('data-link');
	            imagesToLoad.push(newImg);
	            newImg.inject(elm.getElement('.figure-img'), 'top');
	        });
	        //
	        var timeout = 0;
	        ...
	        var time = setInterval(function(){
	            ...
	            if(process == imagesToLoad.length || timeout > 2000){
        		    ...
        		    setTimeout(function() {
        		        var initfig = slides[0].getParent().getParent().getElement('.figcaption');
...
var gk_bluap_autoanimate = function($G, wrapper, dir, next) {
	...
	gk_bluap_animate($G, wrapper, imgs[i], imgs[next]);
	$G['actual_slide'] = next;
