...
class GKIS_gk_fashion_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function checkImages() {
		...
		if($this->config['generate_thumbnails'] == 1) {
			// basic images params		
			$img_width = $this->config['config']->gk_fashion->gk_fashion_image_width;
			$img_height = $this->config['config']->gk_fashion->gk_fashion_image_height;
			$img_bg = $this->config['config']->gk_fashion->gk_fashion_image_bg;
			$quality = $this->config['config']->gk_fashion->gk_fashion_quality;
			// check the slides
			foreach($this->config['image_show_data'] as $slide) {
				$stretch = ($slide->stretch == 'nostretch') ? false : true;
				GKIS_Fashion_Image::createThumbnail($slide->image, $this->config, $img_width, $img_height, $img_bg, $stretch, $quality);	
			}
		}
	}
	// get the articles data
	function getArticleData() {
		...
		// get the data
		if(count($idsK2) > 0) {
			$this->articlesK2 = GKIS_gk_fashion_Model::getDataK2($idsK2);
		}
		if(count($ids) > 0) {
			$this->articles = GKIS_gk_fashion_Model::getData($ids);
		}
	}
	// generate view
	function generateView() {
		...
		// add script fragment
		$document->addScriptDeclaration('try {$Gavick;}catch(e){$Gavick = {};};$Gavick["gkIs-'.$this->config['module_id'].'"] = { "anim_speed": '.$this->config['config']->gk_fashion->gk_fashion_animation_speed.', "anim_interval": '.$this->config['config']->gk_fashion->gk_fashion_animation_interval.', "autoanim": '.$this->config['config']->gk_fashion->gk_fashion_autoanimation.', "slide_links": '.$this->config['config']->gk_fashion->gk_fashion_slide_links.' };');
