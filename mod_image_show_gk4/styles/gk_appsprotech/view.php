	...
	<div class="gkIsImage" style="height: <?php echo $this->config['config']->gk_appsprotech->gk_appsprotech_module_height; ?>px;">
		<?php for($i = 0; $i < count($this->config['image_show_data']); $i++) : ?>
			<?php if($this->config['image_show_data'][$i]->published) : ?>
				...
				<div class="gkIsSlide" style="margin-left: -<?php echo floor($width / 2); ?>px; margin-top: <?php echo $this->config['config']->gk_appsprotech->gk_appsprotech_image_y; ?>px; z-index: <?php echo $i+1; ?>;" title="<?php echo $title; ?>" data-path="<?php echo $path; ?>" data-link="<?php echo $link; ?>"></div>
			<?php endif; ?>
		<?php endfor; ?>
