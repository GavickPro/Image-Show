...
class GKIS_gk_shop_and_buy_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function getArticleData() {
		...
		// get the data
		if(count($idsK2) > 0) {
			$this->articlesK2 = GKIS_gk_shop_and_buy_Model::getDataK2($idsK2);
		}
		if(count($ids) > 0) {
			$this->articles = GKIS_gk_shop_and_buy_Model::getData($ids);
		}
	}
	// generate view
	function generateView() {
		...
		// add script fragment
		$document->addScriptDeclaration('try {$Gavick;}catch(e){$Gavick = {};};$Gavick["gkIs-'.$this->config['module_id'].'"] = { "anim_speed": '.$this->config['config']->gk_shop_and_buy->gk_shop_and_buy_animation_speed.', "anim_interval": '.$this->config['config']->gk_shop_and_buy->gk_shop_and_buy_animation_interval.', "autoanim": '.$this->config['config']->gk_shop_and_buy->gk_shop_and_buy_autoanimation.', "slide_links": '.$this->config['config']->gk_shop_and_buy->gk_shop_and_buy_slide_links.' };');
